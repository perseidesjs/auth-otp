...
import type { ICustomerModuleService, ICacheService, IAuthModuleService } from "@medusajs/framework/types"
...
medusaIntegrationTestRunner({
  testSuite: ({ api, getContainer }) => {
    ...
    const createCustomerWithAuth = async (email: string, firstName: string, lastName: string) => {
    ...
    describe("OTP Auth Routes", () => {
      describe("POST /auth/:actor_type/otp/generate", () => {
        ...
        it("fails with missing identifier", async () => {
          const response = await api.post("/auth/customer/otp/generate", {}).catch((e) => e.response)
      ...
      describe("POST /auth/:actor_type/otp/pre-register", () => {
        it("generates OTP for new identifier (no existing customer)", async () => {
          ...
          const cachedOtp = await cacheService.get(`otp:pre-register:${newEmail}`)
          expect(cachedOtp).toBeDefined()
          expect(cachedOtp).toHaveLength(6)
        })
        ...
        it("fails with missing identifier", async () => {
          const response = await api.post("/auth/customer/otp/pre-register", {}).catch((e) => e.response)
      ...
      describe("Full OTP flow", () => {
        it("completes generate -> verify flow for existing customer", async () => {
          ...
          // Step 1: Generate OTP
          const generateResponse = await api.post("/auth/customer/otp/generate", {
            identifier: customer.email,
          })
